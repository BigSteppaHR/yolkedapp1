...
-- Create function to update user streak if it doesn't exist
...
DROP TRIGGER IF EXISTS workout_completed_trigger ON workout_sessions;
...
SELECT 'Bench Press', 'strength', ARRAY['chest', 'shoulders', 'triceps'], 'barbell', 'Lie on bench, grip bar slightly wider than shoulders, lower to chest, press up.'
WHERE NOT EXISTS (SELECT 1 FROM exercises WHERE name = 'Bench Press');

INSERT INTO exercises (name, category, muscle_groups, equipment, instructions)
SELECT 'Squat', 'strength', ARRAY['legs', 'glutes', 'core'], 'barbell', 'Bar on upper back, feet shoulder width, squat down keeping chest up, drive through heels.'
WHERE NOT EXISTS (SELECT 1 FROM exercises WHERE name = 'Squat');

INSERT INTO exercises (name, category, muscle_groups, equipment, instructions)
SELECT 'Deadlift', 'strength', ARRAY['back', 'legs', 'core'], 'barbell', 'Stand with feet hip width, grip bar outside legs, keep back straight, lift by driving hips forward.'
